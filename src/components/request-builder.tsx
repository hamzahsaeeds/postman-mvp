...
export default function RequestBuilder() {
  ...
  const generateRequestConfig = (): RequestConfig => {
    ...
    if (formData.auth.type === "bearer" && formData.auth.token) {
      ...
      formData.auth.keyName &&
      formData.auth.keyValue
    ) {
  ...
  const onSubmit = async (data: RequestFormData) => {
    ...
    try {
      ...
      toast(
        "Request configuration generated successfully! Check the console for axios config."
      );
    } catch (error) {
  ...
  const getActiveParamsCount = () =>
    watchedValues.params?.filter((p) => p.enabled && p.key).length || 0;
  const getActiveHeadersCount = () =>
    watchedValues.headers?.filter((h) => h.enabled && h.key).length || 0;

  const showBodyWarning = ["GET", "HEAD", "DELETE"].includes(
    watchedValues.method || "GET"
  );
                    ...
                    <FormItem>
                      <FormControl>
                        <Input
                          ...
                          {...field}
                          className={
                            form.formState.errors.url ? "border-error" : ""
                          }
                        />
                      </FormControl>
                      <FormMessage />
